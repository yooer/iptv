...
- **多架构调度**：支持 **MistServer**、**MediaMTX** 与 **TamronOS**，基于权重随机负载均衡。
...
## 二、 运行环境要求
- **操作系统**：Windows 7+ 或 Linux (CentOS 7+, Ubuntu 18.04+, Debian 10+)。
- **数据库**：MongoDB 4.0+ (支持本地或云数据库)。
- **网络**：程序默认监听 `8081` 端口（需在防火墙开启该端口）。
